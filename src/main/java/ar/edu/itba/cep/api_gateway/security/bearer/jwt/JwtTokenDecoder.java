...
public class JwtTokenDecoder implements TokenDecoder {
    ...
    public Mono<BearerTokenAuthentication> decode(final String rawToken) {
        ...
        return Mono.defer(
                () -> Mono.just(rawToken)
                        ...
                        .map(Jws::getBody)
                        .flatMap(JwtTokenDecoder::buildFromClaims)
        );
    }
     ...
     */
    private Mono<Jws<CustomBody>> parseToken(final String token) {
        try {
            final var jws = Jwts.parser().setSigningKey(publicKey).parse(token, jwtHandlerAdapter);
            return Mono.justOrEmpty(jws);
     ...
     */
    private static Mono<BearerTokenAuthentication> buildFromClaims(final CustomBody claims) {
        return Mono.just(
    ...
    private static final class CustomJwtHandlerAdapter extends JwtHandlerAdapter<Jws<CustomBody>> {
         ...
         *                               (i.e no {@link Constants#ROLES_CLAIM} claim).
         * @implNote This method checks the {@link Constants#ROLES_CLAIM} claim in order to get the roles.
         */
